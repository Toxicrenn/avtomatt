...
class subject:
    __scripts = {
        ...
        "edit_subject": text("update subject set subject_title = :new_name where subject_title = :name"),
    ...
    def edit_subject(self,name, new_name):
        conn = self.__db.connect()
        conn.execute(self.__scripts["edit_subject"],{"name": name, "new_name": new_name})
        conn.commit()
        conn.close()
